...
1. 任意のトピックを選んで、ワードクラウドで表現（ただし、品詞を名詞、動詞、形容詞に限定）
	...
	- [みんな夕飯なあに？](http://mamastar.jp/bbs/comment.do?topicId=2904249)
...
1. 検索語とタイプ（T値かMI値か)を引数に、コロケーションの結果を返す関数開発済
1. 日本語の共起分析で、RMeCabライブラリの関数(collocate)を使うと、ストップワードを先に除外してからの共起が算出出来ない。そのため、ストップワード除外は組み合わせペアを出力した後のテーブルを対象にして絞り込みを行っている。しかし、この方法では、***前後5ワード内***の共起語が正しく集計できない。アルゴリズム見直し必要。
	1. 英文ドキュメントの共起語集計は、1つのキーワードを与えて、ストップワードを除いた後の共起語の集計関数は開発した。
	1. ***【未完成】*** 全てのワード間で一気に、ストップワードを事前に除去したワード間共起集計のアルゴリズム
...
1. [トピック全体での「旦那」の共起結果を可視化](https://isyuko.github.io/gh-pages/)
